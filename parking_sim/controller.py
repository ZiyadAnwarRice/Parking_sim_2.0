...
def run_parking_demo(selected_spot="P4"):
    ...
    for i, (wx, wy) in enumerate(waypoints):
        p.addUserDebugText(
            f"W{i+1}", [wx, wy, 0.08],
            textColorRGB=[0.2, 0.9, 1.0], textSize=1.0
        )
        if i < len(waypoints) - 1:
            nx, ny = waypoints[i + 1]
            p.addUserDebugLine(
                [wx, wy, 0.04], [nx, ny, 0.04],
                [0.2, 0.8, 1.0], 2
            )
        ...
        while True:
            ...
            if sim_step % 240 == 0:
                if last_sample_pos is not None:
                    moved = math.hypot(x - last_sample_pos[0], y - last_sample_pos[1])
                    if moved < 0.08:
                        print(f"Stall detected at ({x:.2f}, {y:.2f}), reversing...")
                        for _ in range(180):
                            apply_control(scene.car, scene.wheels, scene.steering,
                                          -MIN_SPEED, 0.0)
                            p.stepSimulation()
                        max_steer_steps = 0
                last_sample_pos = (x, y)
            ...
            # 🔥 FIX: skip waypoint if behind vehicle
            if local_x < -0.05 and target_idx < len(waypoints) - 1:
                target_idx += 1
                continue
            ...
            curvature = STEER_GAIN * (2.0 * local_y / (ld * ld))
            steer = clamp(
            ...
            if abs(steer) >= MAX_STEER - 0.01:
                max_steer_steps += STEPS_PER_SLEEP
            else:
                max_steer_steps = 0

            if max_steer_steps > MAX_STEER_SKIP_STEPS and target_idx < len(waypoints) - 1:
                print(f"Waypoint {target_idx} unreachable, skipping.")
                target_idx += 1
                max_steer_steps = 0
            ...
            if sim_step % 120 == 0:
                print(
                    f"step={sim_step:5d} pos=({x: .2f}, {y: .2f}) "
                    f"target_idx={target_idx} steer={steer: .3f} "
                    f"speed={speed_cmd: .2f} goal_dist={final_dist: .2f}"
                )
